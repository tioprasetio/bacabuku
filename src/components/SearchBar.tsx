...
const SearchBar = () => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (query.trim().length > 0) {
      navigate(`/all-book?keyword=${encodeURIComponent(query.trim())}`);
    }
  };
  ...
  const handleVoiceSearch = () => {
    ...
    recognition.onresult = (event: any) => {
      ...
      setTimeout(() => {
        navigate(
          `/all-book?keyword=${encodeURIComponent(transcript.trim())}`
        );
      }, 500);
    };
  ...
  const handleSuggestionClick = (book: Book) => {
    navigate(`/book/${book.id}-${generateSlug(book.title)}`, {
      state: book,
    });
    setQuery("");
    setSuggestions([]);
